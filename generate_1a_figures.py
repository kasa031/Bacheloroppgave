...
def fig_webbkoll_start():
    ...
    for i, line in enumerate([
        ...
        "Do Not Track off  ·  cookie banner not clicked",
...
def fig_webbkoll_results():
    ...
    ax.text(5, 1.55, "Missing headers: HSTS, CSP, SRI, X-Content-Type-Options, X-Frame-Options.",
            fontsize=9, color=INK, ha="center")
...
def fig_cookie_banner():
    ...
    ax.add_patch(FancyBboxPatch((0.35, 0.35), 9.3, 4.5, boxstyle="round,pad=0.04,rounding_size=0.12",
                                facecolor="white", edgecolor=NAVY, linewidth=1.4))
    ax.text(0.65, 4.4, "skatteetaten.no/person/", fontsize=11, color=NAVY, fontweight="bold")
    ax.text(0.65, 3.95, "Optional web statistics cookies", fontsize=14, color=INK, fontweight="bold")
    ax.text(0.65, 3.4, "Necessary cookies stay on. Optional statistics cookies (Skyra, Matomo,",
            fontsize=9, color=MUTED)
    ax.text(0.65, 3.05, "Google Analytics, Siteimprove) require a choice.", fontsize=9, color=MUTED)

    ax.add_patch(FancyBboxPatch((0.65, 1.35), 3.9, 1.15, boxstyle="round,pad=0.03,rounding_size=0.08",
                                facecolor=NAVY, edgecolor=NAVY, linewidth=0))
    ax.text(2.6, 1.92, "Ja", fontsize=16, color="white", ha="center", va="center", fontweight="bold")
    ax.add_patch(FancyBboxPatch((5.45, 1.35), 3.9, 1.15, boxstyle="round,pad=0.03,rounding_size=0.08",
                                facecolor=CREAM, edgecolor=NAVY, linewidth=1.6))
    ax.text(7.4, 1.92, "Nei  (selected)", fontsize=15, color=NAVY, ha="center", va="center", fontweight="bold")
    ax.text(5, 0.75, "Recorded 24 August 2026. Tests optional tracking after rejection; not the basis for tax processing.",
            fontsize=8, color=MUTED, ha="center")

    path = OUT / "fig7_skatteetaten_cookie_banner.png"
    fig.savefig(path, bbox_inches="tight", facecolor=PAPER)
    plt.close()
    print("wrote", path)
