#!/usr/bin/env python3
"""Build ACIT4100 research-field essay HTML (APA 7 / Kildekompasset)."""
...
add("What this research field studies", 2, [
    "Human factors cybersecurity research examines how people perceive, evaluate, and respond to security threats in everyday digital environments. Phishing and social engineering are central because they exploit communication channels users already trust: email, SMS, messaging apps, and voice calls (Conteh & Schmick, 2020; Hadnagy, 2018).",
    "Typical research objects include susceptibility to deceptive messages, effectiveness of awareness training, reporting behaviour after near misses, cognitive and emotional predictors of unsafe actions, and organisational culture around blame and disclosure (Chen et al., 2024; Parker & Flowerday, 2020). Norwegian studies by Tjostheim and Waterworth (2020, 2022) exemplify national contributions linking cognitive reflection to phishing compliance in representative samples.",
    "The field does not treat \"the user\" as a homogeneous category. Recent work emphasises lure-specific vulnerability: people may reject stranger scams yet hesitate on messages impersonating institutions, peers, or employers (Klütsch et al., 2024). That line of inquiry connects to broader debates about digital trust in high-trust societies such as Norway, where BankID, Altinn, and university IT communication create both convenience and impersonation risk (Finans Norge, 2026; Norwegian National Security Authority [NSM], 2026).",
])

add("Notable researchers, groups, and centres", 2, [
    "Human factors security research is distributed globally rather than centred in one laboratory. Influential researchers include Christopher Hadnagy on social engineering practice and ethics (Hadnagy, 2018); Steven Pinker on classic prose style used in science communication (Pinker, 2014); and scholars such as Parker and Flowerday (2020) and Tsai (2021) on phishing persuasion and organisational context.",
    "Institutional clusters include Carnegie Mellon's CyLab and usable security traditions; the Symposium on Usable Privacy and Security (SOUPS) community; European Union Agency for Cybersecurity (ENISA) research networks; and Nordic contributions through Norwegian ICT conferences where Tjostheim and Waterworth publish representative-sample phishing studies (Tjostheim & Waterworth, 2020, 2022).",
    "Industry research units publish threat data that academia cites for motivation but rarely for method. Students mapping the field should distinguish corporate report series from peer-reviewed archives (European Union Agency for Cybersecurity [ENISA], 2024; Halcyon, 2026).",
])

add("How to read and approach scientific work in this field", 2, [
    "Course guidance emphasises skill in locating and reading scientific sources rather than producing anthropology from scratch. A practical workflow: define the field boundary (human factors and phishing); search ACM Digital Library and IEEE with terms such as \"phishing susceptibility,\" \"social engineering,\" and \"usable security\"; skim abstracts for method type; read methods and limitations before discussion sections.",
    ...
    "Students are not required to judge statistical quality at expert level, but reporting how authors interpret their own findings is expected. Phrases such as \"the authors argue\" or \"the study suggests, within undergraduate samples\" keep interpretation honest, as recommended in academic writing guidance (Pinker, 2014).",
])

add("Regulatory and professional constraints in Norway", 2, [
    "Norwegian researchers must align with the Norwegian Agency for Shared Services in Education and Research (SIKT) notification or approval requirements for personal data on behaviour and sensitive topics such as loneliness and fraud victimisation. National campaigns by Finans Norge (2026) and NSM (2026) shape public scepticism but are not research instruments—students should not confuse policy documents with empirical studies.",
    "Working life and education law intersect when organisational phishing simulations affect employees or students. Punitive simulation designs have been criticised in research literature for suppressing reporting (Chen et al., 2024; Bayl-Smith et al., 2024)—the regulatory environment for such programmes is organisational policy as much as national law.",
])

add("Effects of this research upon society", 2, [
    "When effective, human factors research changes training content, incident response playbooks, and public messaging. When ignored, organisations continue blaming users while impersonation of BankID, Altinn, and university IT persists (Norwegian National Security Authority [NSM], 2026; Norwegian Police, 2024).",
    "Social effects include reduced shame when non-punitive reporting is normalised (Chen et al., 2024); potential stigmatisation if loneliness is framed as individual risk rather than structural student-life issue; and improved digital literacy when lure-specific examples replace generic \"don't click\" slogans (Finans Norge, 2026).",
    "The research field does not operate in a vacuum: student wellbeing surveys (SHoT Study, 2022) and cybersecurity awareness modules rarely cross-reference each other, yet both speak to student vulnerability in digital environments (Directorate of Health, 2024; Statistics Norway, 2025). Describing that silo is part of describing the field's societal embedding.",
])

add("Style note: classic and narrative hybrid", 2, [
    "This essay applies classic style in the main exposition: direct statements, active verbs, concrete nouns, and sequential reasoning as Pinker (2014) recommends for clarifying complex ideas to a general educated reader. Narrative cut-scenes function like documentary B-roll: they do not introduce new empirical claims but situate the reader emotionally before returning to synthesis of published research.",
...
add("Experimental and laboratory designs", 2, [
    "Controlled experiments remain common. Participants receive simulated phishing emails or messages in lab or online settings; researchers manipulate variables such as sender identity, urgency, authority cues, or grammatical quality (Tsai, 2021). Dependent variables include click rates, credential entry, time to decision, and self-reported suspicion.",
    "Strengths include isolation of causal factors, reproducibility, and statistical power. Weaknesses include simulated attacks that may not carry real consequences and participant awareness of being studied, which can inflate cautious behaviour (social desirability bias). Ethics boards therefore scrutinise deceptive designs carefully (Chen et al., 2024).",
])
...
add("Surveys and scenario vignettes", 2, [
    ...
    "This approach scales nationally, as Tjostheim and Waterworth (2020, 2022) demonstrated in Norway. Trade-off: intended behaviour may differ from behaviour under real loss or embarrassment.",
])
...
add("Field studies and industry collaboration", 2, [
    ...
    "Threat intelligence reports from ENISA (2024), Microsoft (2024), and NSM (2026) provide macro context but rarely substitute for peer-reviewed human-subjects research.",
])
...
add("Philosophy of science in the field", 1, [
    ...
    "Positivist and post-positivist strands dominate experimental and survey work: hypotheses, measurable variables, falsification, effect sizes. Protection Motivation Theory from health psychology migrated into security behaviour research under this logic (Floyd et al., 2000; Rogers, 1975; Bayl-Smith et al., 2024).",
    "Interpretivist and constructivist strands appear in interview-based and ethnographic work: meaning is situated; \"risk\" is co-produced by users, institutions, and attackers. A message is not objectively phishing until interpreted in context (Braun & Clarke, 2006).",
    ...
    "The field rarely resolves these philosophies into one camp. Pragmatism wins: mixed methods prevail because the phenomenon is too messy for a single epistemology (Fetters et al., 2013).",
])
...
add("History and development of the field", 1, [
    ...
    "The shift toward human factors accelerated as defences improved and attackers pivoted to social manipulation. Hadnagy's (2018) social engineering literature and annual industry reports documented that technical maturity did not eliminate human entry points.",
    "Human-computer interaction venues such as CHI and SOUPS began accepting phishing behaviour papers alongside usable security workshops. Psychology and consumer fraud research on loneliness and persuasion cross-pollinated with cybersecurity during the 2010s (Alves & Wilson, 2008; Cacioppo & Patrick, 2008).",
    ...
    "Recent trajectories include AI-assisted lure generation (Halcyon, 2026; Trend Micro, 2026), loneliness and student wellbeing links (SHoT Study, 2022; Statistics Norway, 2025), and scepticism paradox framing—why general caution toward strangers coexists with institutional impersonation vulnerability (Klütsch et al., 2024).",
])

add("Publication traditions: journals and conferences", 1, [
    "Researchers in this field publish across security, HCI, and organisational journals. Notable outlets include <em>Computers &amp; Security</em>, <em>Computers in Human Behavior</em>, <em>Information and Computer Security</em>, <em>Journal of Cybersecurity</em>, and proceedings from CHI, SOUPS, and USENIX Security workshops (Parker &amp; Flowerday, 2020; Tsai, 2021).",
    ...
    "Grey literature—NSM risk assessments, ENISA threat landscapes, Finans Norge fraud campaigns—shapes public discourse and student motivation but does not replace peer review for academic essays. Students learning the field must read both: reports for timeliness, journals for methodology transparency (ENISA, 2024; NSM, 2026).",
])

add("Ethics, regulation, and codes of conduct", 1, [
    "Ethics is not peripheral in this field; it is structural. Deceptive experiments (simulated phishing without consent) can harm participants through shame, misplaced blame, or unintended disclosure of personal habits (Chen et al., 2024).",
    "Institutional review boards and national agencies such as SIKT in Norway evaluate whether deception is justified, whether debriefing is adequate, and whether data minimisation is observed. Live phishing against unwitting participants is generally rejected; vignettes and announced simulations are preferred.",
    "The General Data Protection Regulation governs storage of behavioural data. Fraud victimisation questions require content warnings. Reporting near misses in organisational studies must protect employee identity (Alves & Wilson, 2008).",
    "Professional codes in cybersecurity emphasise lawful behaviour and public good, but academic research adds informed consent and right to withdraw. Tension arises when industry partners want identifiable incident data researchers cannot ethically provide (Hadnagy, 2018).",
])

narrative("""
The ethics form asks: \"Will participants be deceived?\" She checks \"minimal deception—scenario vignettes only.\" The committee asks for a debrief link to svindel.no (Finans Norge, 2026). Outside, autumn rain on the campus windows. Research here moves slowly—not because computers are slow, but because people matter.
""")
...
add("Relationship with society, industry, and other experts", 1, [
    ...
    "Relations with \"hard\" security research are complementary. Cryptographers and malware analysts ask what is technically possible; human factors researchers ask what is psychologically plausible. Incident responders supply anecdotes; academics supply sampling frames and statistics (ENISA, 2024).",
    "Disagreement exists. Some practitioners dismiss academic effect sizes as irrelevant to security operations centre workload; some academics criticise punitive phishing simulations in workplaces (Bayl-Smith et al., 2024; Chen et al., 2024). The research field documents both sides rather than enforcing unity.",
])
...
add("How researchers produce and synthesise knowledge", 2, [
    ...
    "Quality signals include pre-registration for experiments, open instruments, representative sampling, preregistered analysis plans, and replication attempts. Weak signals include convenience samples without boundary discussion, click-rate-only metrics without context, and fear-based training evaluations without coping measures (Bayl-Smith et al., 2024).",
    "Meta-analyses on protection motivation and security behaviour aggregate decades of small studies (Floyd et al., 2000). Narrative reviews in ENISA and industry reports aggregate incident trends without human-subjects detail. A well-written field essay cites both types while respecting their limits.",
])

add("Current debates within the field", 2, [
    "Active debates include whether simulated phishing in workplaces improves behaviour or increases shame (Chen et al., 2024); whether AI-generated lures change psychology or only aesthetics (Halcyon, 2026; Trend Micro, 2026); whether loneliness predicts fraud susceptibility equally across age groups (Klütsch et al., 2024; Wen et al., 2024); and whether general scepticism training transfers to institutional impersonation.",
...
add("Conclusion: communicating research as technologists", 1, [
    "Human factors cybersecurity research is a field where technologists learn to read humans with the same rigour they read code—through methods, ethics, history, and public consequence. It explains why phishing survives technical progress: attackers optimise for trust, urgency, and belonging, not only for software bugs (Hadnagy, 2018; Tsai, 2021).",
    "For ACIT students, understanding this field means knowing where knowledge comes from, not only what the latest awareness poster says. It means recognising journals, ethics constraints, and societal stakeholders. It means writing about research in clear classic prose, occasionally illuminated by narrative cut-scenes that remind us that behind every dataset is someone hovering over a link between lectures (Pinker, 2014).",
    "The field will keep evolving as digital trust infrastructures evolve. Norway's combination of high institutional trust and measurable social disconnection among young adults makes it a relevant site for contemporary work—not because Norwegians are uniquely weak, but because the research questions about scepticism, loneliness, and credible impersonation are unusually visible here (NSM, 2026; Statistics Norway, 2025).",
...
# APA 7 reference list (Kildekompasset): alphabetical, hanging indent 1.27 cm, DOI/URL per type
REFERENCES = """
...
<p class="refs hanging">Pinker, S. (2014). <em>The sense of style: The thinking person's guide to writing in the 21st century</em>. Viking.</p>
...
<p class="refs hanging">Tsai, H.-Y. (2021). Phishing and persuasion strategies [Article manuscript]. <em>Computers &amp; Security</em>.</p>
...
<p class="refs note">Referanselisten er formatert etter APA 7th edition (Kildekompasset). Kontroller alle DOI-er, forfatterlister og URL-er i Zotero/Ordbok før endelig innlevering.</p>
"""
...
.page { max-width:210mm; margin:1rem auto; background:var(--paper); padding:25mm 25mm 25mm 25mm; box-shadow:0 2px 12px rgba(0,0,0,.15); }
.cover { text-align:center; min-height:160mm; display:flex; flex-direction:column; justify-content:center; page-break-after:always; line-height:2; }
...
h1.ch { font-size:12pt; font-weight:700; text-align:center; margin:2rem 0 1rem; page-break-before:always; }
h1.ch:first-of-type { page-break-before:auto; text-align:left; border-bottom:none; }
h1.refs-heading { text-align:center; border-bottom:none; }
h2 { font-size:12pt; font-weight:700; margin:1.2rem 0 .6rem; color:var(--black); text-align:left; }
p { font-size:12pt; margin:0 0 .65rem; text-align:left; text-indent:0.5in; }
h1 + p, h2 + p, .abstract p:first-of-type, .narrative p, .criteria p, .refs, .note { text-indent:0; }
.narrative { margin:1.2rem 0; padding:1rem 1.2rem; background:var(--narr); border-left:4px solid var(--dark); font-style:italic; line-height:1.5; }
.narrative-label { font-size:10pt; font-style:normal; font-weight:700; color:var(--mid); margin-bottom:.4rem; text-indent:0; }
...
criteria = """
<div class="criteria"><strong>Referansestil:</strong> APA 7th edition (Kildekompasset). Dobbelt linjeavstand (2.0). Hengende innrykk i referanseliste (0,5 tommer / 1,27 cm). Henvisninger: forfatter–år i parentes. Organisasjonsforkortelser definert ved første bruk (f.eks. NSM, ENISA).</div>
"""
...
<p class="keywords"><em>Keywords:</em> human factors, phishing, social engineering, research methods, APA, cybersecurity behaviour</p>
"""
...
<body>
<div class="toolbar">
  <strong>ACIT4100 Essay · APA 7 (Kildekompasset)</strong> (~{word_count} words)
  · <a href="ACIT4100_research_field_essay.pdf">PDF</a>
  · <a href="../masterthesis/index.html">Master drafts</a>
</div>
<div class="page">
<div class="cover">
  <h1>{TITLE}</h1>
...
out = Path("/workspace/acit4100/ACIT4100_research_field_essay.html")
out.write_text(html)
